...
/*
 * Our SpiffyAuthNode object defines our server and routines it needs
...
var SpiffyAuthNode = {
    ...
    runServer: function() {
            ...
            path.exists(filename, function(exists) {  
                ...
                if (!exists) {  
                    if (uri.indexOf('/simple/') === 0) {
                    	 ...
                    	 */
                    	if (SpiffyAuthNode.isAuth(request.headers['authorization'])) {
                            /*
                             * Authorized!
                             */
                    	...
                    	if (request.method === 'POST') {
                    		...
                    		SpiffyAuthNode.tokens.push(token);
                        	response.writeHeader(200, {'Content-Type': 'application/json'});  
                        	response.end(JSON.stringify(json)); 
                    		return;            		
                    	} else if (request.method === 'DELETE') {
                    		var authHeader = request.headers['authorization'];
                    		SpiffyAuthNode.removeToken(authHeader);
                    		response.writeHeader(200, {'Content-Type': 'application/json'});  
                    		var json = {'Status': 'OK'};
                    		response.end(JSON.stringify(json));
                    		return;
                         ...
                         */
                        var resource  = SpiffyAuthNode.getI18nDateResource(request, uri);
        ...
        fs.readdir(path.join(process.cwd(), targetDir,  i18nDir), function(err, files) {
            SpiffyAuthNode.resources = files;
    ...
    getI18nDateResource: function(request, uri) {
        ...
        for (i = 0; i < locales.length; i++) {
            ...
            if (SpiffyAuthNode.resourceExists(resource)) {
                return resource;
            } else {
                /*
                 * Try just on country
    ...
    resourceExists: function(resource) {
        var i = 0;
        for (i = 0; i < SpiffyAuthNode.resources.length; i++) {
            if (SpiffyAuthNode.resources[i] === resource) {
                return true;
            }
        }
        return false;
    ...
    isAuth: function (authHeader) {
    	var i = 0;
    	if (authHeader != null) {
    		for (i = 0; i < SpiffyAuthNode.tokens.length; i++) {
    			if ('X-OPAQUE ' + SpiffyAuthNode.tokens[i] === authHeader) {
    				return true;
    			}
    		}
    	}
